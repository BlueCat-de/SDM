...
def main(argv):
    ...
    replay_buffer = ReplayBuffer(num_state, num_action_ego, num_action_adv, FLAGS.replay_buffer_size, device=FLAGS.device) 
    ...
    if FLAGS.pretrain_ego:
        ...
        if FLAGS.load_pretrain_ego:
            model_pre_ego.load(FLAGS.pretrain_ego_path)
        ...
        # if FLAGS.save_model:
        #     torch.save(model_pre_ego.state_dict(), os.path.join(eval_savepath, 'models', 'pretrain_ego.pt'))
        ...
        sampler_pretrain_ego_policy = deepcopy(sampler_pretrain_ego_policy)
    # return
    # TODO: apply cross learning method
    replay_buffer.reset()
    sampler_ego_policy.set_grad(False)
    sampler_adv_policy.set_grad(False)
        ...
        for epoch in trange(FLAGS.n_epochs):
            ...
            with Timer() as train_timer:
                ...
                for batch_idx in trange(FLAGS.n_train_step_per_epoch): # at each step of a epoch:
                    ...
                    # freeze_ego = False
                    # freeze_adv = batch_idx % FLAGS.n_adv_policy_update_gap == 0 and batch_idx != 0
                    if FLAGS.used_wandb:
                        wandb_logger.log(prefix_metrics(model_ego.train(batch, user='ego'), 'SAC_ego'))
                        wandb_logger.log(prefix_metrics(model_adv.train(batch, user='adv'), 'SAC_adv'))
                    else:
                        metrics.update(prefix_metrics(model_ego.train(batch, user='ego'), FLAGS.model_name))
                        metrics.update(prefix_metrics(model_adv.train(batch, user='adv'), FLAGS.model_name))
                    # if batch_idx % FLAGS.n_ego_policy_update_gap == 0 and batch_idx != 0:
