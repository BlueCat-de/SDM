...
parser.add_argument('--r_adv', type = str, default = 'stackelberg')
...
parser.add_argument('--n_loops', type = int, default = 6)
...
parser.add_argument('--project', type = str, default = 'clearning')
...
def main(argv):
    ...
    replay_buffer = GradReplayBuffer(num_state, num_action_ego, num_action_adv, FLAGS.replay_buffer_size, device=FLAGS.device) \
        if FLAGS.model_name == 'SDM' else ReplayBuffer(num_state, num_action_ego, num_action_adv, FLAGS.replay_buffer_size, device=FLAGS.device)
    ...
    if FLAGS.model_name == 'SDM':
        model = SDM(None, 
                       ego_policy = adv_policy, 
                       adv_policy = ego_policy, 
                       qf1_ego = qf1_adv,
                       qf2_ego = qf2_adv,
                       target_qf1_ego = target_qf1_adv,
                       target_qf2_ego = target_qf2_adv,
                       qf1_adv = qf1_ego,
                       qf2_adv = qf2_ego,
                       target_qf1_adv = target_qf1_ego,
                       target_qf2_adv = target_qf2_ego,
                       device = FLAGS.device,
    ...
    if FLAGS.pretrain_ego:
        ...
        for i in range(FLAGS.pretrain_loop):
            ...
            for epoch in trange(FLAGS.pretrain_epochs):
                pretrain_sampler.env.adv_policy = FLAGS.adv_policy
    ...
    freeze_ego = False
    freeze_adv = False
    last_metric_av_crash = -1
    last_metric_bv_crash = 2
        ...
        for epoch in trange(FLAGS.n_epochs):
            ...
            with Timer() as train_timer:
                ...
                for batch_idx in trange(FLAGS.n_train_step_per_epoch): # at each step of a epoch:
                    ...
                    if FLAGS.used_wandb:
                        wandb_logger.log(prefix_metrics(model.train(batch, freeze_ego = freeze_adv, freeze_adv = freeze_ego), FLAGS.model_name))
                    else:
                        metrics.update(prefix_metrics(model.train(batch, freeze_ego = freeze_adv, freeze_adv = freeze_ego), FLAGS.model_name))
                    # ipdb.set_trace()
                    # if batch_idx % FLAGS.n_ego_policy_update_gap == 0 and batch_idx != 0:
            ...
            with Timer() as eval_timer:
                if epoch == 0 or (epoch + 1) % FLAGS.eval_period == 0:
                    ...
                    for ego_policy in ['sumo', 'fvdm', 'RL']: # this RL ego is pretrained ego
                        ...
                        #     else:
                        #         freeze_adv = False
                        #     last_metric_bv_crash = metrics[f'{ego_policy}_{eval_adv_policy}/metrics_bv_crash']
