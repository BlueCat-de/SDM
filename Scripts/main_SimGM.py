...
parser.add_argument('--r_adv', type = str, default = 'stackelberg2')
...
parser.add_argument('--n_adv_policy_update_gap', type = int, default = 1)
parser.add_argument('--n_ego_policy_update_gap', type = int, default = 1)
parser.add_argument('--model_name', type = str, default = 'RARL')
...
parser.add_argument('--n_train_step_per_epoch', type = int, default = 50)
...
parser.add_argument('--pretrain_ego_path', type = str, default = '/home/qh802/cqm/Cross-Learning/Scripts/output/bv=5-sumo_r-ego=stackelberg_r-adv=stackelberg3_seed=42_time=2023-08-28-20-03-22reg_scale=0.2_pretrain_adv=sumo/models/pretrain_ego.pth')
parser.add_argument('--reset_rb', type = str, default = 'True', choices = ['True', 'False'])
...
def main(argv):
    ...
    replay_buffer_ego = ReplayBuffer(num_state, num_action_ego, num_action_adv, FLAGS.replay_buffer_size, device=FLAGS.device)
    replay_buffer_adv = ReplayBuffer(num_state, num_action_ego, num_action_adv, FLAGS.replay_buffer_size, device=FLAGS.device)
    ...
    model_adv = SAC(
        ...
        qf1 = qf1_adv,
        qf2 = qf2_adv,
        target_qf1 = target_qf1_adv,
        target_qf2 = target_qf2_adv
    )
    model_ego.torch_to_device(FLAGS.device)
    model_adv.torch_to_device(FLAGS.device)
    ...
    replay_buffer_adv.reset()
    replay_buffer_ego.reset()
    freeze_ego = False
    freeze_adv = False
        ...
        for epoch in trange(FLAGS.n_epochs):
            ...
            with Timer() as train_timer_ego:
                ...
                train_sampler.sample(
                    ...
                    deterministic=False, replay_buffer=replay_buffer_ego,
                    joint_noise_std=FLAGS.joint_noise_std
                ...
                for batch_idx in trange(FLAGS.n_train_step_per_epoch): # at each step of a epoch:
                    batch = replay_buffer_ego.sample(FLAGS.batch_size) # Draw actions a_t^1, a_t^2 from their distributions pi_1, pi_2
                    if FLAGS.used_wandb:
                        wandb_logger.log(prefix_metrics(model_ego.train(batch, user='ego'), 'ego'))
                    else:
                        metrics.update(prefix_metrics(model_ego.train(batch, user='ego'), 'ego'))

            with Timer() as train_timer_adv:
                train_sampler.env.adv_policy = "RL"
                train_sampler.env.ego_policy = "RL"
                if FLAGS.reset_rb:
                    replay_buffer_adv.reset()
                train_sampler.sample(
                    ego_policy=sampler_ego_policy, adv_policy=sampler_adv_policy, n_steps=FLAGS.n_rollout_steps_per_epoch,
                    deterministic=False, replay_buffer=replay_buffer_adv,
                    joint_noise_std=FLAGS.joint_noise_std
                ) # Sample trajectories from the simulator using the current policy pi_1, pi_2
                for batch_idx in trange(FLAGS.n_train_step_per_epoch): # at each step of a epoch:
                    batch = replay_buffer_adv.sample(FLAGS.batch_size) # Draw actions a_t^1, a_t^2 from their distributions pi_1, pi_2

                    # at the end of each step, train the policy and Q function
                    freeze_ego = not ((batch_idx % FLAGS.n_ego_policy_update_gap) == 0 and batch_idx != 0)
                    freeze_adv = not ((batch_idx % FLAGS.n_adv_policy_update_gap) == 0 and batch_idx != 0)
                    if FLAGS.used_wandb:
                        wandb_logger.log(prefix_metrics(model_adv.train(batch, user='adv'), 'adv'))
                    else:
                        metrics.update(prefix_metrics(model_adv.train(batch, user='adv'), 'adv'))
            ...
            if FLAGS.save_model and FLAGS.used_wandb:
                save_data = {FLAGS.model_name: 'RARL',
                            'variant': variant, 'epoch': epoch}
                wandb_logger.save_pickle(save_data, 'model.pkl')
            # metrics['rollout_time'] = rollout_timer()
            metrics['train_time_ego'] = train_timer_ego()
            metrics['train_time_adv'] = train_timer_adv()
            metrics['eval_time'] = eval_timer()
            metrics['epoch_time'] = train_timer_ego() + eval_timer() + train_timer_adv()
            if FLAGS.used_wandb:
                    wandb_logger.log(metrics)
    ...
    if FLAGS.save_model and FLAGS.used_wandb:
        ...
        wandb_logger.save_pickle(save_data, 'model.pkl')
    if FLAGS.save_model:
        torch.save(model_ego, os.path.join(eval_savepath, 'models', f'trained_ego.pth'))
        torch.save(model_adv, os.path.join(eval_savepath, 'models', f'trained_adv.pth'))
