...
def main(argv):
    ...
    replay_buffer.reset()
    # add data of pretrain replay buffer into replay buffer
    actions = [(action_ago, action_adv) for action_ago, action_adv in \
        zip(torch.Tensor(pretrain_replay_buffer.action_ego), torch.Tensor(pretrain_replay_buffer.action_adv))]
    for i in trange(pretrain_replay_buffer.size):
        replay_buffer.append(
            pretrain_replay_buffer.state[i], actions[i], 
            pretrain_replay_buffer.reward[i], pretrain_replay_buffer.next_state[i], 
            pretrain_replay_buffer.done[i]
        )
