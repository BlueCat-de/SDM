...
run_task() {
    ...
    # command
    command="taskset -c 1-128 python main_SDM.py --device $device --used_wandb True --is_save True --save_model True --num_agents $num_agents --reg_scale $reg_scale --seed $seed --r_adv stackelberg3 --n_adv_policy_update_gap $n_adv_policy_update_gap --n_ego_policy_update_gap $n_ego_policy_update_gap"
    # run command
    $command
}
        ...
        for seed in "${seed_values[@]}"; do
            for n_adv_policy_update_gap in "${n_adv_policy_update_gap_values[@]}"; do
                for n_ego_policy_update_gap in "${n_ego_policy_update_gap_values[@]}"; do
                    # filter out
                    if [ $n_adv_policy_update_gap != 5 ] || [ $n_adv_policy_update_gap != 5 ]; then
                        # GPU allocate
