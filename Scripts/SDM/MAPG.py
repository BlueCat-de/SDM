...
from leaderUpdate import compute_leader_grad, leader_step, adam_grad, compute_stackelberg_grad
from objective import objective_function
from utils import network_param_index, calc_gradient_norm
from SimpleSAC.models.model import soft_target_update
...
class MAPG(object):
    ...
    def __init__(self, config, ego_policy, qf1_ego, target_qf1_ego, qf2_ego, target_qf2_ego, adv_policy, qf1_adv, target_qf1_adv, qf2_adv, target_qf2_adv, regularization = 10000., device = 'cuda:1', reg_scale = 0):
        self.config = MAPG.get_default_config(config)
        ...
        self.opt_ego = torch.optim.Adam(self.ego_policy.parameters(), lr=self.lr_ego, betas=(self.beta1, self.beta2))
        self.sch_ego = torch.optim.lr_scheduler.ExponentialLR(self.opt_ego, gamma=self.gamma_ego)
    ...
    def train(self, batch : dict, freeze_ego = False, freeze_adv = False):
        ...
        q_new_actions_adv = torch.min(
            self.qf1_adv(observations, new_actions_ego, new_actions_adv),
            self.qf2_adv(observations, new_actions_ego, new_actions_adv)
        )
        alpha_loss = observations.new_tensor(0.0)
        alpha = observations.new_tensor(self.config.alpha_multiplier)

        ego_policy_loss = ((alpha * log_pi_ego) - q_new_actions_ego).mean()
        adv_policy_loss = ((alpha * log_pi_adv) - q_new_actions_adv).mean()

        # ipdb.set_trace()
        q1_pred_ego = self.qf1_ego(observations, actions_ego, actions_adv)
        q2_pred_ego = self.qf2_ego(observations, actions_ego, actions_adv)
        q1_pred_adv = self.qf1_adv(observations, actions_ego, actions_adv)
        q2_pred_adv = self.qf2_adv(observations, actions_ego, actions_adv)
        ...
        # optimize ego
        if not freeze_ego:
            self.opt_ego.zero_grad()
            ego_policy_loss.backward(retain_graph = True)
            self.opt_ego.step()
