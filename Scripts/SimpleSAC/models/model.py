...
def multiple_action_q_function(forward):
    ...
    def wrapped(self, observations, actions_ego, actions_adv, is_x_grad=False, **kwargs):
        # ipdb.set_trace()
        ...
        q_values = forward(self, observations, actions_ego, actions_adv, is_x_grad, **kwargs)
        # if multiple_actions:
...
class FullyConnectedNetwork(nn.Module):
    def __init__(self, input_dim, output_dim, arch='256-256', orthogonal_init=False, is_LN='', is_SN=False):
        super().__init__()
...
class TanhGaussianPolicy(nn.Module):
    ...
    def __init__(self, observation_dim, action_dim, arch='256-256',
        ...
        self.base_network = FullyConnectedNetwork(
            observation_dim, 2 * action_dim, arch, orthogonal_init, is_LN=is_LN, is_SN=is_SN
        )
    ...
    def forward(self, observations, deterministic=False, repeat=None):
        ...
        # TODO: if test, deteministic=True
        return self.tanh_gaussian(mean, log_std, deterministic)


class SamplerPolicy(object):

    def __init__(self, policy, device, grad = False):
        self.policy = policy
        self.device = device
        self.grad = grad
...
class FullyConnectedQFunction(nn.Module):
    ...
    def __init__(self, observation_dim, action_ego_dim, action_adv_dim, arch='256-256', orthogonal_init=False,
                 is_LN='3', is_SN=False):
        super().__init__()
        self.observation_dim = observation_dim
        ...
        self.network = FullyConnectedNetwork(
            observation_dim + action_ego_dim + action_adv_dim, 1, arch, orthogonal_init, is_LN=is_LN, is_SN=is_SN
        )

    # @multiple_action_q_function
    def forward(self, observations, actions_ego, actions_adv, is_x_grad=False):
        # ipdb.set_trace()
        input_tensor = torch.cat([observations, actions_ego, actions_adv], dim=-1)
        output = self.network(input_tensor)
        if is_x_grad:
            input_tensor_grad = input_tensor.detach().requires_grad_(True)
            output_grad = self.network(input_tensor_grad)  # 与网络输出分别计算，避免二者的梯度影响，但是降低了效率 TODO 增加这条语句后的测试
